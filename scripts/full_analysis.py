...
class PublicParser(argparse.ArgumentParser):
    ...
    INTERNAL = ("next-work", "audit", "submit-result", "record-usage", "submit-correction", "rework", "mark-failed", "self-check")
...
def parser() -> argparse.ArgumentParser:
    ...
    # 质量归属：每个 skill 产出后的自我校验（Goal 1）。复用 Gate 的 _substance_errors
    # 对该 skill 的 lean 契约阈值做确定性校验；通过才进入 mk_result_bundle / submit-result。
    sc = sub.add_parser("self-check", help=argparse.SUPPRESS)
    sc.add_argument("--run-root", required=True); sc.add_argument("--registry", default=gate.DEFAULT_REGISTRY)
    sc.add_argument("--skill-id", required=True); sc.add_argument("--report", required=True)
...
def main(argv=None) -> int:
    ...
    try:
        ...
        if args.command == "next-work":
            emit(runtime.next_work(root, methodology_mode=args.methodology_mode)); return 0
