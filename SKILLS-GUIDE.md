...
个别 skill 文件正文写的输出路径是 `~/xxx.md` 或 `local/reports/` 根目录，与项目 CLAUDE.md 的报告命名表不完全一致。**在本仓库内执行时以 CLAUDE.md 命名表为准**：公司相关报告进 `local/reports/{公司名}/` 文件夹，行业/主题/组合报告放 `local/reports/` 根目录。新增报告后跑 `python3 scripts/build_report_index.py` 重建索引。
...
- **产出**：`local/reports/{公司名}/{公司名}-research-{YYYYMMDD}.md`，结论须给明确买入/观望/回避与具体价格区间
...
- **产出**：`local/reports/{公司名}/` 目录——README + 01-04 四视角分报告 + `最终报告.md`
...
- **产出**：`local/reports/{公司名}/{公司名}-management-{YYYYMMDD}.md`
...
- **产出**：`local/reports/{公司名}/{公司名}-private-{YYYYMMDD}.md`，含六维评分、估值区间（保守/合理/乐观 + 安全边际）、一页纸投资决策表、信息盲区地图
...
- **产出**：`local/reports/{公司名}/《看懂{公司名}》-{YYYYMMDD}/` 目录，每篇 `0X-标题.md`，另有 `00-系列说明.md` 目录索引；旧版目录已存在时不覆盖不混放
...
- **产出**：`local/reports/{公司名}/{公司名}-earnings-{期间}.md`
...
- **产出**：全在 `local/reports/{公司名}/`——`{公司名}-earnings-{期间}.md`（定稿）+ `-研究底稿.md` + 四视角分报告（`-段永平/-巴菲特/-芒格/-李录.md`）+ `-读者评审.md`
...
- **产出**：`local/reports/{行业名}-industry-{YYYYMMDD}.md`（reports/ 根目录）
...
- **产出**：`local/reports/{行业名}-funnel-{YYYYMMDD}.md`（reports/ 根目录）
...
- **产出**：全在 `local/reports/bottleneck-map/`——完整扫描 `{趋势名}-bottleneck-{YYYYMMDD}.md`、每日扫描 `daily/`、持续维护 `master-map.md` 与 `watchlist.md`
...
- **产出**：`local/reports/{公司名}/{公司名}-checklist-{YYYYMMDD}.md`；结论四态——通过/未通过/灰色地带/N-A
...
/portfolio-review 我的持仓          # 需已有 local/reports/portfolio-latest.md
```
...
- **产出**：持续更新单一文件 `local/reports/portfolio-latest.md`（非按日期建新文件）；结论必须明确回答组合健康度、最应该做的一件事、当前最大风险
...
- **产出**：长期维护 `local/reports/{公司名}/{公司名}-thesis.md` 单文件
...
/thesis-drift 腾讯 local/reports/腾讯-thesis-20250101.md reports/腾讯-thesis-20260701.md
```
...
- **产出**：`local/reports/{公司名}/{公司名}-news-{YYYYMMDD}.md`
