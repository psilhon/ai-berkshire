...
- **ingest 写入边界**：manifest、artifact 记录与 provenance 先在内存中完整准备，准备阶段失败不再提前复制 attempt 产物到正式路径。
...
### 🧪 测试 (Tests)
- `bash scripts/check.sh` 全绿（450 单元测试 + 14 个 skill frontmatter 治理 + Codex/WorkBuddy 生成物同步 + Contract v2 的 13 项契约校验 + 报告索引检查）。
