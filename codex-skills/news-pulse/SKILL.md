...
🔴 STOP / 检查点：在启动 4 个后台侦察 Agent 自主联网取数前，必须先向用户确认（给出明确选项：如"确认启动新闻脉搏侦察团队""仅先规划侦察任务不启动"），获得明确同意后再继续；未经确认不得自主执行。
...
5. 严格区分"事实"与"推测"，遵循数据客观性原则（所有判断附数据来源，区分事实与观点）
...
🔴 STOP / 检查点：在将完整异动归因报告写入用户文件系统或对外发布前，必须先向用户确认（给出明确选项：如"确认写入 local/reports/{公司名}/{公司名}-news-YYYYMMDD.md""仅生成草稿供预览不落盘"），获得明确同意后再继续；未经确认不得自主落盘或外发。
...
- 不要硬凑因果链：找不到主因时明确写"真因不明"，这比编造因果更有价值（市场可能在抢跑利空）。
- 不要被标题党误导：标题与正文不符的事件必须标注"标题误导"，不得把传言当事实。
- 不要仅凭单源下结论：传言类信息至少 2 个独立来源验证，单一小作文不得作为归因依据。
- 不要预设立场：不因持仓就倾向"这是情绪波动没事"，证据指向哪边写哪边。
- 不要在用户未授权时落盘或外发：完整归因报告写入 `local/reports/` 与发布必须经过 D4 检查点获得明确同意。

---

## 失败处理（如果 X 失败 → Y）

- 如果 某个后台侦察 Agent 超时/拒答/无返回 → 执行 向用户展示已收到的维度并标注缺失维度，必要时主代理补做该维度侦察，不假装"完整归因"。
- 如果 WebSearch/WebFetch 取数失败或受限 → 执行 明确告知用户"部分维度未能联网侦察"并标注"数据缺失/未核验"，不编造事件。
...
本 skill 执行异动归因时应优先使用以下 Tushare 命令获取可审计的资金面和情绪数据。

| 命令 | 数据内容 | 在本 skill 中的使用方式 | 层级 |
|------|---------|----------------------|:--:|
| `top-list` | 龙虎榜（每日 106+ 条） | 异动归因——查看标的是否进龙虎榜、机构买卖方向 | **必须** |
...
| 依赖项 | 路径 | 用途 | 可达性 |
|--------|------|------|--------|
| xueqiu_scraper.py（可选） | `tools/xueqiu_scraper.py` | 抓取关键意见领袖帖子 | ✅ |
| ashare_data.py（可选） | `tools/ashare_data.py` | 行情数据获取 | ✅ |
