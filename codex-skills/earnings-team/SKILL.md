...
🔴 STOP / 检查点：在并行启动编辑 Agent 与读者评审 Agent 前，必须先向用户确认（给出明确选项：如"确认启动编辑+评审""仅生成研究报告初稿不润色"），获得明确同意后再继续；未经确认不得自主派发这两个后台 Agent。
...
🔴 STOP / 检查点：在把四大师报告、合成研究报告与公众号文章写入 `local/reports/{公司名}/` 前，必须先向用户确认（给出明确选项：如"确认写入并发布""仅生成草稿不落盘"）；含"操作建议/四大师会怎么做"等强结论时同样须过此检查点，获得明确同意后再继续；未经确认不得自主落盘或下投资结论。
...
🔴 STOP / 检查点：在跑 `report_audit.py` 准出并对外发布公众号文章前，必须先向用户确认（给出明确选项：如"确认发布""仅生成草稿待审阅"），获得明确同意后再继续；未经确认不得自主发布。
...
- 不要跳过读者评审的"必须修改"项直接发布：定稿须逐条处理硬伤，不得为省事略过。
- 不要编造数据：编辑改写必须保留全部关键数据，关键数字须 financial_rigor.py 验算，不准为可读性牺牲准确性。
- 不要只唱多或只唱空：四大师视角须相互印证与挑战，结论须附反面论据。
- 不要在用户未授权时把公众号文章落盘或对外发布：写入 `local/reports/` 与准出须经 D4 检查点。
...
- 如果 `financial_rigor.py` 验算报错 → 执行：核对数值单位/币种后重试，不把报错当已验证。
- 如果 `report_audit.py` 判【证据不足】/【打回】→ 执行：补齐第二来源或修正后重跑，不绕过审计发布。
- 如果 编辑/读者评审 Agent 不可用 → 执行：由 Team Lead 主会话按角色手动改写并自审，明确告知未独立复核。
- 如果 用户输入含糊（如缺公司名/期间）→ 执行：列出缺失项请用户补全，不自选默认值展开。
