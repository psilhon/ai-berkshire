...
🔴 STOP / 检查点：在启动后台 Agent 自主联网取数前，必须先向用户确认（给出明确选项：如"确认启动全部公司并行取数""仅先规划不取数"），获得明确同意后再继续；未经确认不得自主执行。
...
🔴 STOP / 检查点：在输出"通过/未通过/灰色地带"等 Checklist 强结论前，必须先向用户确认（给出明确选项：如"输出完整结论与对比表""仅输出中性分析不替用户下结论"），获得明确同意后再继续；未经确认不得自主替用户决定投资与否。
...
🔴 STOP / 检查点：在将完整 Checklist 报告写入用户文件系统前，必须先向用户确认（给出明确选项：如"确认写入~/巴菲特Checklist-*.md""仅生成草稿供预览不落盘"），获得明确同意后再继续；未经确认不得自主落盘。
...
- 不要用单一信源/估算填满六关表格：ROE、毛利率、FCF 等关键数字若无法从可靠信源获取，应标注"数据不足/估计"，严禁把推测值伪装成真实数据。
- 不要把"资料少"误判为"看不懂"或"否决"：C级公司应诚实标注为"灰色地带"，不得因 AI 无法填满表格就一律判为不通过。
- 不要跳过 `tools/financial_rigor.py` 的市值/估值验算：所有涉及计算的数据必须工具验算，禁止 LLM 心算后假装精确。
- 不要用模糊的"可能还行"替代镜子测试的明确结论：5句话说不完整就标注"不买"，结论必须可执行。
- 不要在用户未授权时落盘或外发：完整报告写入 `~/巴菲特Checklist-*.md` 必须经过 D4 检查点获得明确同意。
- 🔴 **不要产出薄摘要**：每个关卡必须展开分析（具体数据、判断逻辑、风险评估），单关缺论证直接标注该关为"分析不完整"。汇总表格不能替代逐关展开论证。
...
- 如果 Task 工具启动的后台取数 Agent 超时/拒答/无返回 → 执行 降级为当前主代理直接调用数据管线或手动搜索取数，并明确告知用户"部分数据由主代理补充，置信度见标注"，不静默跳过。
- 如果 `tools/financial_rigor.py` 验算报错或偏差过大（❌）→ 执行 排查单位/口径错误（如市值单位、AB股），修正后重算；仍失败则标注"数据未核验"且不在结论中使用该数字。
- 如果 数据抽检发现双源冲突（误差>1%）→ 执行 在报告中同时标注两源数字与分歧，标为"证据不足"并打回重审，不强行合并。
- 如果 用户输入含糊（如只给公司名未给范围）→ 执行 列出 2-3 个可能的筛选范围请用户澄清，不在未确认时自选展开。
...
| 依赖项 | 路径 | 用途 | 可达性 |
|--------|------|------|--------|
| financial_rigor.py | `tools/financial_rigor.py` | 估值验证与三轮情景建模（市值验算/估值核验） | ✅ |
