...
| `signals <代码>` | 龙虎榜/资金流/解禁/融资融券四块信号（两融块当前不可用，见陷阱7） | `--date YYYY-MM-DD`（仅龙虎榜，且只在深交所备用源路径真正按日过滤） | 东财 → 新浪（资金流）/深交所（深市龙虎榜） |
...
7. **融资融券块当前不可用**：东财接口标识符是未替换的占位符（`market_signals.py` 的 reportName），恒返回"未找到融资融券记录"——标"数据不足"时注明这是工具已知缺陷，不是市场无两融数据。
