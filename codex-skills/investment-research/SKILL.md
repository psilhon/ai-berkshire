...
🔴 STOP / 检查点：在启动后台 Agent 自主联网取数前，必须先向用户确认（给出明确选项：如"确认启动后台 Agent 取数""仅先规划数据清单不取数"），获得明确同意后再继续；未经确认不得自主执行。
...
🔴 STOP / 检查点：在输出"空仓者/持仓者/卖出信号/加仓信号"等强投资建议前，必须先向用户确认（给出明确选项：如"输出完整买入/观望/回避建议""仅输出中性分析不荐股"），获得明确同意后再继续；未经确认不得自主替用户决定投资方向。
...
🔴 STOP / 检查点：在将完整研究报告写入用户文件系统或对外发布/外发前，必须先向用户确认（给出明确选项：如"确认写入并发布~/[公司名]投资研究报告.md""仅生成草稿供预览不落盘"），获得明确同意后再继续；未经确认不得自主落盘或外发。
...
- **【准出】**：全部抽检点双源核验且偏差 ≤ 1% → 报告可发布
- **【证据不足】**：有未核验/单一来源/两源冲突点 → 补齐第二来源后重跑
- **【打回】**：任意点与所填全部信源偏差 > 1% → 修正对应数据后重新抽检，直到准出

---

## 反例与红线（不要做）

- 不要用单一信源下投资结论：任一财务/估值判断都必须至少双源交叉验证，单一来源不得作为结论依据。
- 不要编造财务数据：收入、净利润、现金储备、市值等数字若无法从可靠信源获取，应标注"未知/暂缺"，严禁估算后伪装成真实数据。
- 不要跳过 `tools/financial_rigor.py` 的市值/估值/三情景验算：所有涉及计算的数据必须工具验算，禁止 LLM 心算。
- 不要把 C级信息稀缺误判为"不好"：C级应列出"需要一手验证的问题清单"，不得用推测填满框架伪装确定性。
- 不要在用户未授权时外发或落盘：完整研究报告写入 `~/` 与发布必须经过 D4 检查点获得明确同意。

---

## 失败处理（如果 X 失败 → Y）

- 如果 后台取数 Agent 超时/拒答 → 执行 降级为当前主代理直接按 `skills/financial-data.md` 规范取数，并明确告知用户"部分数据由主代理补充"，不静默跳过。
- 如果 `tools/financial_rigor.py` 验算 ❌ 偏差过大（如市值单位错误、口径差异）→ 执行 排查单位/口径后重算；仍失败则标注"数据未核验"且不在结论中使用。
- 如果 `tools/report_audit.py` 抽检【打回】或【证据不足】→ 执行 补齐第二来源或修正对应数据后重跑，直到【准出】；不强行发布未核验报告。
- 如果 WebSearch/WebFetch 取数失败 → 执行 明确告知用户"无法取数"并标注"数据缺失/未核验"，不编造或假装成功。
- 如果 用户输入含糊/多义（如仅给公司名无范围）→ 执行 列出 2-3 个可能的解读请用户澄清，不在未确认时自选展开。
...
本 skill 应优先使用以下 Tushare 结构化命令获取可审计可冻结的数据。合约 feeds 映射定义了这些命令与本 skill 的归属关系。

| 命令 | 数据内容 | 在本 skill 中的使用方式 | 层级 |
|------|---------|----------------------|:--:|
| `income-stmt` | 利润表原始数据（85 列） | 扣非利润精算、营业利润/投资收益逐项拆解 | 必须 |
| `balance-sheet` | 资产负债表 | 商誉规模、存货结构、应收账款账龄、有息负债 | 必须 |
...
| `broker-recommend` | 券商月度金股 | 机构推荐池（可选） | 可选 |
| `cyq-chips` | 筹码分布 | 筹码集中度、主力成本区间 | 可选 |
| `stk-factor` | 量化因子 | 换手率/量比/PE 分位日频数据 | 可选 |
| `unblock` | 限售股解禁 | 近期解禁压力 | 可选 |
| `block-trade` | 大宗交易 | 大资金动向 | 可选 |
| `weekly/monthly` | 周线/月线行情 | 中长期趋势判断 | 可选 |
| `suspend` | 停复牌信息 | 流动性风险 | 可选 |
| `name-history` | 历史证券名称变更 | 公司更名（如韦尔→豪威）追踪 | 可选 |
| `holder-num` | 股东户数趋势 | 筹码集中/分散趋势 | 可选 |
| `research-visits` | 机构调研记录 | 机构关注度 | 可选 |
