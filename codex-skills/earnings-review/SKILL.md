...
━━━ 排雷前置评分卡 ━━━━━━━━━━━━━━━━━━━
  ...
  L0硬门槛: {通过/直接排除}
  财报可信度等级: {A/B/C/D}   ← A.6 否决门（C→最高观望 / D→规避）
...
### A.6 财报可信度评级（A/B/C/D 否决门，估值前置）

> 来源移植：本块方法论取自外部项目 `rollingSirius/equity-research-skill` 的 `references/forensic-accounting.md`（MIT），经本项目纪律改写并接入现有排雷评分卡。它解决的核心问题：排雷评分卡只回答「能不能碰」，但没回答「这份利润是不是真的、值不值得折现」——估值前置的质量门缺失，会让「估值便宜」对冲掉造假信号。

目的：在「能不能碰」（排雷评分卡 L0–L6）之外，先回答**「这份利润值不值得折现」**。本块与 A.1–A.5 **共用同一批结构化数据**（`ashare_data.py` 的 `income-stmt`/`balance-sheet`/`cash-flow`/`announcements`），产出 **财报可信度等级 A/B/C/D**，直接约束最终投资动作——这是估值的前置硬门，不与排雷评分卡重复：排雷回答「雷」，可信度回答「利润真实性」。

**计算项（能算则算，取不到写"未获取到"，不许凭印象打分）**：
- **应计质量**：总应计比率 = (净利润 − 经营现金流) ÷ 平均总资产；现金转化率 = CFO ÷ 净利润（看 3–5 年趋势，长期 <80% 且下滑为红旗）。数据来自 `cash-flow` + `balance-sheet`。对应排雷 L4.1（经营CF/净利润<1）、L4.5（净利↑+FCF负）。
- **Beneish M-Score 八变量代理**（本项目无现成脚本，按三表手算或清单判定）：DSRI=应收/收入指数↑（对应 L3.1）、GMI=毛利率指数↑（对应 L1 毛利异常）、AQI=非流动非PP&E占比↑、SGI=收入增长指数↑、DEPI=折旧率指数↑、SGAI=费用率指数↑（对应 L1.5）、TATA=总应计/总资产↑、LVGI=杠杆指数↑。M > −1.78 落入「可能操纵」区，移入逐项手工核查。
- **收入确认红旗**：DSO 背离（应收增速持续>收入增速，L3.1 同）、递延收入背离（订阅类致命）、季末突击/渠道压货、重述或换收入确认政策。
- **治理红旗**：审计非四大/频繁更换/非标（L0 已覆盖）、CFO 突辞（L5.3）、监管立案（L5.9）、关联交易与资金占用（L5.8）、股权质押高（A股治理红线）。
...
| B | 1–2 项轻度红旗且有合理解释 | 无限制，列入监控指标 |
| C | M-Score 越限、DSO/递延持续背离、经常性"非经常"、治理红旗 1 项严重 | **动作最高"观望"**，报告第一章须警示 |
| D | 多项严重红旗叠加 / 涉嫌造假 / 审计非标 | **规避**，估值章改为"可信度恢复前无意义" |

> 评级与逐项证据写入报告「财报质量」小节（无则单列）；等级 C/D 时**禁止用"估值便宜"对冲**——便宜是造假最常见的伪装。本门与排雷评分卡 L0 直接排除同源互斥：任一触发即 D/排除。

### A.7 🔴 STOP / 检查点（可信度否决前置）

在阶段二给出投资动作前，必须先依据 A.6 的 A/B/C/D 等级执行否决：C → 动作不得高于"观望"；D → 直接"规避"，估值章改写为"可信度恢复前无意义"。未评级（数据不足）时按 B 处理但明示"可信度未核验"。
...
## 一、一句话结论
> 排雷风险等级：{低/中/高/极高/直接排除}；**财报可信度等级：{A/B/C/D}**（C→最高观望 / D→规避）；50-100字：超/符/低预期，核心变化，对投资论文的影响。
...
## 九、结论
> 🔴 财报可信度否决门（A.6）：动作先过 A/B/C/D。C→最高"观望"；D 或 L0 直接排除→"规避"，估值/买入论述改写"可信度恢复前无意义"，禁止用"估值便宜"对冲。
1. 超/符/低预期？
...
- 不要跳过 A.6 财报可信度否决门：C/D 等级必须执行动作约束（C→最高观望、D→规避），不得用"估值便宜""赛道好"对冲利润真实性红旗；未评级（数据不足）按 B 处理但明示"可信度未核验"。
