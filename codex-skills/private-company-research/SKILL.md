...
🔴 STOP / 检查点：在启动 6 个后台 Agent 自主联网取数前，必须先向用户确认（给出明确选项：如"确认启动未上市公司研究团队""仅先展示任务规划不启动"），获得明确同意后再继续；未经确认不得自主执行。
...
🔴 STOP / 检查点：在输出"投资/观望/回避"等强投资建议与真实价值区间前，必须先向用户确认（给出明确选项：如"输出完整投资结论与估值区间""仅输出中性分析不荐股"），获得明确同意后再继续；未经确认不得自主替用户决定投资方向。
...
🔴 STOP / 检查点：在将完整最终研究报告写入用户文件系统或对外发布前，必须先向用户确认（给出明确选项：如"确认写入 local/reports/{公司名}/{公司名}-private-YYYYMMDD.md""仅生成草稿供预览不落盘"），获得明确同意后再继续；未经确认不得自主落盘或外发。

### 第八步：保存报告

将完整最终报告写入 `local/reports/{公司名}/{公司名}-private-{YYYYMMDD}.md`。
...
- 不要编造未上市公司的财务数据：无标准财报时，每个数字必须标注来源、时间、置信度（🟢/🟡/🔴），搜不到就标"数据缺失"，严禁用"合理推测"伪装成有据分析。
- 不要虚假精确：信息稀缺公司应切换"第一性原理模式"聚焦核心问题，留白标注"不知道"，不得用推测填满模板。
- 不要强行与上市公司对标继承其估值逻辑：未上市公司有独特价值与折扣，忽略这点会高估。
- 不要正面信息详细、负面信息简略：汇总阶段必须做跨维度信号一致性检验与反面检验。
- 不要在用户未授权时落盘或外发：完整研究报告写入 `local/reports/` 与发布必须经过 D4 检查点获得明确同意。

---

## 失败处理（如果 X 失败 → Y）

- 如果 某个后台 Agent 超时/拒答/无返回 → 执行 向用户展示已收到的维度并标注缺失维度，必要时主代理补做，不假装"完整研究"。
- 如果 WebSearch/WebFetch 取数失败或受限 → 执行 明确告知用户"部分维度未能联网"，标注"数据缺失/未核验"，不编造。
- 如果 关键数据仅单一来源（孤证）或来源冲突 → 执行 在报告中列出全部来源并标注差异与采信判断，标为"低置信度/待核实"，不强行合并。
- 如果 信号一致性检验发现矛盾（如增长叙事 vs 招聘缩减）→ 执行 在报告中显式标注矛盾并要求进一步验证，不掩盖。
- 如果 用户输入含糊/多义（如仅给公司名）→ 执行 列出 2-3 个可能的公司/范围解读请用户澄清，不在未确认时自选展开。
