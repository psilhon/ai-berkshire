...
关键不变式：
1. 合规 run（产物充足余量 + 全覆盖心跳）→ verdict=PASS，退出码 0；
2. 坍塌 run（全部分析单元压到下限 + 零心跳）→ verdict=WARN，含"零 heartbeat"告警；
...
def _build_run(root: Path, floors: dict, *, size_overrides=None,
               statuses=None, heartbeat_skills=None, skip_events=False,
               corrupt_events=False, partial_bad_lines=0,
               attempt_heartbeats=None, accepted_attempt_ids=None) -> Path:
    """构造最小可诊断 run。

    - size_overrides: {skill_id: size_mult}，控制每单元字节= floor*mult
    - statuses: {skill_id: status}，默认全部 PASS；NOT_APPLICABLE 单元不产物
    - heartbeat_skills: 发心跳的 skill_id 集合（每单元一条 skill 级 heartbeat，无 attempt_id）
    - attempt_heartbeats: [(skill_id, attempt_id)] 精确到 attempt 的心跳
    ...
    heartbeat_skills = heartbeat_skills or []
    attempt_heartbeats = attempt_heartbeats or []
    ...
    if skip_events:
        return root
    events = [{"type": "job_started", "work_unit_id": f"wu-job{i}"} for i in range(20)]
    if corrupt_events:
        (root / "evidence/events.jsonl").write_text(
            "not-json\n{broken\n", encoding="utf-8")
        return root
    for sid in heartbeat_skills:
        events.append({"type": "heartbeat", "work_unit_id": f"wu-{sid}"})
    for sid, aid in attempt_heartbeats:
        events.append({"type": "heartbeat", "work_unit_id": f"wu-{sid}", "attempt_id": aid})
...
class DoctorTests(unittest.TestCase):
    ...
    def test_compliant_run_verdict_pass(self):
        ...
        _build_run(self.root, self.floors, heartbeat_skills=ids)  # 全覆盖心跳 + 2x 余量
        ...
        self.assertGreaterEqual(report["heartbeat_coverage"], doctor.COVERAGE_FLOOR)

    def test_cliff_run_verdict_warn(self):
        size_ov = {sid: 1.0 for sid in self.floors}  # 全部压线
        _build_run(self.root, self.floors, size_overrides=size_ov, heartbeat_skills=[])
        report = doctor.diagnose(self.root, REGISTRY)
        self.assertEqual(report["verdict"], "WARN")
        self.assertTrue(any("零 heartbeat" in w for w in report["warnings"]), report["warnings"])
        self.assertGreaterEqual(report["thin_share_analytic"], 0.40)

    def test_na_units_excluded_from_thin(self):
        # 7 个分析单元 N/A，其余 2x 余量 + 全覆盖心跳 → 应 PASS（N/A 不算贴线）
        ...
        _build_run(self.root, self.floors, statuses=na, heartbeat_skills=rest)
        report = doctor.diagnose(self.root, REGISTRY)
    ...
    def test_events_missing_warns(self):
        ids = list(self.floors)
        _build_run(self.root, self.floors, heartbeat_skills=ids, skip_events=True)
        report = doctor.diagnose(self.root, REGISTRY)
    ...
    def test_events_corrupt_warns(self):
        ids = list(self.floors)
        _build_run(self.root, self.floors, heartbeat_skills=ids, corrupt_events=True)
        report = doctor.diagnose(self.root, REGISTRY)
    ...
    def test_events_partial_warns(self):
        ...
        ids = list(self.floors)
        _build_run(self.root, self.floors, heartbeat_skills=ids, partial_bad_lines=2)
        report = doctor.diagnose(self.root, REGISTRY)
        self.assertEqual(report["events_status"], "partial")
        self.assertEqual(report["verdict"], "WARN")
        self.assertTrue(any("部分损坏" in w for w in report["warnings"]))

    def test_stale_attempt_heartbeat_does_not_cover_accepted(self):
        """P0：旧失败 attempt 的心跳不得替零心跳的 accepted attempt 背书。"""
        ids = list(self.floors)
        # accepted attempt 统一为 attempt-final；但心跳全部打在旧 attempt-old 上
        accepted = {sid: "attempt-final" for sid in ids}
        stale_hb = [(sid, "attempt-old") for sid in ids]
        _build_run(self.root, self.floors, accepted_attempt_ids=accepted,
                   attempt_heartbeats=stale_hb)
        report = doctor.diagnose(self.root, REGISTRY)
        # 虽然 hb_total>0（有旧心跳），但 accepted attempt 覆盖应为 0 → 全程零心跳告警
        self.assertEqual(report["heartbeat_coverage"], 0.0)
        self.assertTrue(any("零 heartbeat" in w for w in report["warnings"]),
                        report["warnings"])
        self.assertEqual(report["verdict"], "WARN")

    def test_matching_attempt_heartbeat_covers_accepted(self):
        """对照组：心跳打在 accepted attempt 上 → 覆盖成立 → PASS。"""
        ids = list(self.floors)
        accepted = {sid: "attempt-final" for sid in ids}
        good_hb = [(sid, "attempt-final") for sid in ids]
        _build_run(self.root, self.floors, accepted_attempt_ids=accepted,
                   attempt_heartbeats=good_hb)
        report = doctor.diagnose(self.root, REGISTRY)
        self.assertEqual(report["heartbeat_coverage"], 1.0)
        self.assertEqual(report["verdict"], "PASS", report["warnings"])
    ...
    def test_advisory_exit_code_zero_by_default(self):
        _build_run(self.root, self.floors,
                   size_overrides={s: 1.0 for s in self.floors}, heartbeat_skills=[])
    ...
    def test_strict_exit_code_zero_on_pass(self):
        _build_run(self.root, self.floors, heartbeat_skills=list(self.floors))
        self.assertEqual(doctor.main(["--run-root", str(self.root),
