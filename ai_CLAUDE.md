...
- 校验揪出两个真工具缺陷并如实写进 skill 数据陷阱：signals 两融块 reportName 是未替换占位符（恒"未找到记录"）、financials 北交所号段误判（已修，含 920 新号段迁移）——教训：数据类 skill 的每条能力声明都要拿实测退出码/源码验证，不能照工具 docstring 转述
