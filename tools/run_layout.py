...
注意边界：本模块只管"东西放在哪"，**不管"谁来写"**——状态所有权仍归
gate（登记账本 / 事件）与 runtime（work unit 状态机），不在本模块收拢。
...
# ---- 运行时状态三件套（路径在此单点定义，所有权分属 gate / runtime）----
MANIFEST_REL = Path("evidence/00-analysis-manifest.json")   # gate 写（经 ingest-result）
