...
工作流程（三步）：
  ...
  一步完成（仅提取+打印抽检清单，不做网络验证）：
    python3 tools/report_audit.py extract --report local/reports/xxx.md --dry-run
"""
...
工作流程：
  ...
  Step 1 — 提取数据点并随机抽样 15%，输出抽检清单：
    python3 tools/report_audit.py extract --report local/reports/腾讯/腾讯-research-20260408.md
  ...
  指定抽样比例（默认0.15）：
    python3 tools/report_audit.py extract --report local/reports/xxx.md --ratio 0.20

  固定随机种子（复现同一批样本）：
    python3 tools/report_audit.py extract --report local/reports/xxx.md --seed 42
        """)
