...
- **层内并行**：同层内互不依赖、只依赖上游层的契约应 fan-out，不必顺序。可并行组：Layer 1 `financial-data` ∥ `quality-screen` ∥ `investment-checklist`（ashare-data 数据取齐后）；Layer 2 `investment-research` ∥ `management-deep-dive` ∥ `earnings-review`（三条互不依赖的基线）；Layer 3 `industry-research` ∥ `industry-funnel` ∥ `bottleneck-hunter` ∥ `news-pulse`；Layer 4 `thesis-tracker` ∥ `thesis-drift` ∥ `portfolio-review` ∥ `private-company-research`（论文与组合互不依赖）；Layer 5 三个内容契约。这些可并行组的执行形状由下条「可用平台强制真独立并行」规则裁定。
- **可用平台强制真独立并行**：平台存在子代理工具且当前授权允许时（预检第 4 步判定），层内互不依赖的多视角契约**必须**以真独立子代理 fan-out，不得退化为单上下文顺序执行。单上下文仅是**无子代理能力平台**的显式 fallback（记 `execution_mode=sequential_single_context`、`assurance=SINGLE_CONTEXT`），不是可用平台的默认。逐契约派发数与角色见下文「子代理派发配方」。
...
- **条件增强预取**：`init` 冻结的 `tushare_configured=true` 时，Layer 1 就把跨层要用的 Tushare 条件命令取好（如 `peers` 供 Layer 3 候选池），下游就近引用、不重复取数；未配置时必须记录 `tushare_not_configured` limitation（映射见 Layer 1 取数口径）。
...
- `init` 在每项 manifest 的 `assigned_artifacts` 中为目标路径分配稳定 artifact ID。`finish-skill --evidence-file` 必须为 ashare 数据底座及其注册表 `feeds` 消费者提交 `artifact_records`：每条只含 `artifact_id`、`artifact_path`、`input_artifact_ids`、`fact_ids`、`command_ids`。ashare 记录连接共享核心事实和全部成功 command ID；下游记录同时引用 ashare artifact ID 与其消费 operation 的 command ID。悬空 ID、后向/同层依赖或缺 feeds 映射均由 gate 判 FAIL。
- 后序产物必须通过 artifact ID 引用前序产物，禁止覆盖或复用同一目标路径。

### Layer 1 取数口径

**基础集（零依赖，必取）：** A 股标的的 `ashare-data` 在全量分析语境下不止默认概览三件套：另取 `history`、`equity-history`、`announcements`、`signals`。先把现行证券代码通过 `init --codes` 冻结；`begin-skill ashare-data` 后，每个注册 operation 都必须调用 `python3 tools/full_analysis_gate.py run-ashare-command --registry tools/full_analysis_contract.json --run-root <run_root> --operation <op> --code <code> --source <publisher-or-feed>`，多来源可重复 `--source`。该入口由 gate 构造 argv、实际执行 CLI，并把 stdout/stderr 与 SHA-256 冻结到 `evidence/commands/`；非零退出仍留失败收据。`finish-skill` 对 ashare-data **不接受调用者提交** `command_receipts`，部分成功不得吞警告。非 A 股命中注册表谓词后走 `financial-data` 替代路径并落负向验收。

**Tushare 增强集（`init` 冻结 `tushare_configured=true` 时必取；未配置必须记一条 `tushare_not_configured` limitation）：** `ashare-data` 的 Tushare-primary 独立主数据源，取到即写入共用证据底座，供下游就近引用与双源核对。**"命令 → 消费 skill → 采集层"的完整清单是注册表 `conditional_command_operations.values` 的唯一机器真源**（`capability=tushare_configured`，每项含 `op`/`feeds`/`layer`；直接读注册表取用），本文不复述；各命令语义见 `skills/ashare-data.md`。配置时缺命令或全部收据 `exit_code != 0` 都由 gate 判 FAIL，不能以 advisory 名义跳过。两个承载编排语义的要点：`mainbz` 是分部收入的独立第二源（喂 financial-data 做分部双源核对），`peers` 是申万成员候选池（Layer 1 预取、Layer 3 喂 industry-funnel 候选池自动化）。大盘级估值分位用 `index-val` 作背景锚，不入公司证据底座、不登记条件命令。
...
- `investment-research` 是**唯一基线**；`investment-team` 在**可用平台默认以 4 个独立子代理**执行（`interpreter-duan` / `interpreter-buffett` / `interpreter-munger` / `interpreter-li` 分别从段永平、巴菲特、芒格、李录方法出发找反证），做反证与增量整合，不重做基线、不重复全量取数；`independent_context_count>=2` 时正常 PASS 并升 assurance。其最终报告必须逐章保留“段永平视角、巴菲特视角、芒格视角、李录视角、四视角对照表、分歧仲裁、综合结论”，不得用通用业务维度替代或合并四位命名视角。**仅当平台无子代理能力**时，才以 `NOT_APPLICABLE_PASS` 收口并引用基线（记录 `sequential_single_context` 限制）——这是能力缺失下的诚实降级，不是默认路径，不得顺序生成同质报告冒充。
- `earnings-review` 是**唯一事实底稿**；`earnings-team` 同理**默认以 6 个独立子代理**（4 大师 + editor + reader）消费冻结底稿做多视角解释、编辑与读者复核；**仅当平台无子代理能力**时才以 N/A 收口并引用底稿。
...
- **默认真独立**：平台工具存在且授权允许时，多视角契约默认创建真实独立角色会话（每角色一个子代理，独立取数与判断），`finish-skill` 如实记录 `independent_context_count`；不根据平台名称假设，按预检第 4 步实测能力判定。
- **仅无能力时降级**：平台无子代理工具或授权不允许时，才由主会话按角色顺序完成，记录 `execution_mode=sequential_single_context`、保障等级降为 `SINGLE_CONTEXT`，不得声称并行或独立复核；不得在有能力平台以"省 token"为由跳过真独立。
