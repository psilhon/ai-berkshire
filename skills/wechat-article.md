...
2. **不用AI腔调**。禁止"让我们一起一起来看看"、"值得关注的是"、"不得不说"等套话
...
> 🔴 STOP / 检查点：在把未取得来源或敏感数据写入对外文章前，必须先向用户确认（给出明确选项：是否使用、如何标注来源），获得明确同意后再继续；未经确认不得自主执行。
...
> 🔴 STOP / 检查点：在对外发布/推送文章前，必须先向用户确认（给出明确选项：发布到哪个公众号/账号、是否立即推送），获得明确同意后再继续；未经确认不得自主执行。
>
> 🔴 STOP / 检查点：在调用任何外部发布 API 前，必须先向用户确认（给出明确选项：目标平台、账号、发布时间），获得明确同意后再继续；未经确认不得自主执行。
...
| 类型 | 路径 | 命名格式 |
|------|------|---------|
| 技术主题 | `local/reports/AI产业研究/` | `公众号-{主题关键词}-{YYYYMMDD}.md` |
| 投资主题 | `local/reports/{公司名}/` | `{公司名}-公众号-{YYYYMMDD}.md` |
| 通用主题 | `local/reports/` | `公众号-{主题关键词}-{YYYYMMDD}.md`

---

## 失败处理（如果 X 失败 → Y）

- 如果素材取数/研究 Agent 失败或返回空 → 执行：用已核实的素材续写，对缺失部分明确标注"存疑/待核实"，不要编造数据或引语。
- 如果作者/编辑/读者 Agent 拒答或超时 → 执行：先用当前已有稿件继续，并提示用户该环节未完成，由用户决定是否重试或跳过。
- 如果发布 API 调用失败 → 执行：保留本地定稿文件，向用户返回失败原因与重试选项，不得反复自动重试或绕过确认直接发布。
- 如果字数/格式不符合目标平台要求 → 执行：按平台约束调整结构或压缩（如超字数则精简案例），并提示用户改动点，不擅自改变核心论点。

---

## 反例与红线（不要做）

- 不要把未核实的财务/技术数字写进对外文章；搜不到来源就标注"估计"或省略，绝不编造。
- 不要编造用户观点、专家引语或案例；引用必须可追溯到真实来源。
- 不要把"题材/假设"当"事实"陈述；推测性内容要明确标注为推断。
- 不要在用户未明确授权时对外发布或推送到任何公众号/平台账号。
- 不要制造标题党或误导性标题来骗取点击；标题要准确反映文章内容。
...
| 依赖项 | 路径/来源 | 用途 | 可达性 |
|--------|------|------|--------|
| Task / WebSearch / Agent | WorkBuddy 内置 | 作者-编辑-读者三 Agent 协作、研究搜索 | ✅ 运行时托管 |
| pdftoppm | 系统工具（poppler-utils） | PDF 页面渲染为高分辨率 PNG（≥900 DPI） | ⚠️ 需系统安装 `poppler` |
| PIL / Pillow | Python 库 | 图表裁切与图像处理 | ⚠️ 需 `pip install Pillow` |

> **自检**：Agent 内置能力由 WorkBuddy 托管；`pdftoppm` 和 `PIL` 仅在"阶段四-第三步（含图表论文处理）"触发时需要，非核心路径。若环境中未安装 poppler/Pillow，该图表提取步骤降级为手动描述。新增依赖需同步更新本清单。
