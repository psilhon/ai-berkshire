...
3. **零依赖层无日K/OHLC 价格序列**：`kline` 命令（Tushare daily+adj_factor，前复权）已补此缺口但需 `TUSHARE_TOKEN`（Tier 0）；**无 token 时仓库不提供内建脚本**，需自行取数（如 `pip install akshare` 走腾讯源）——原 `tools/akshare_data.py` 因依赖未安装、零测试、从未被实际调用，已于 v3.10.13 移除（裁定见 `docs/adr/0001` 的 2026-09-04 修订）。历史价格分析的复权口径规则见 `skills/financial-data.md`「股价与复权」。
