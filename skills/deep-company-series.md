...
🔴 STOP / 检查点：在把系列文章写入 `local/reports/{公司名}/《看懂{公司名}》/` 前，必须先向用户确认（给出明确选项：如"确认写入新文件夹""覆盖前先备份旧版""仅生成草稿不落盘"），获得明确同意后再继续；若旧版系列已存在，未经确认不得覆盖或混放。
...
🔴 STOP / 检查点：在派 Explore agent 并行扫描全系列做跨篇一致性核对前，必须先向用户确认（给出明确选项：如"确认启动一致性扫描""仅先人工抽检关键数字"），获得明确同意后再继续；未经确认不得自主派发后台 agent。
...
🔴 STOP / 检查点：在 `git pull --rebase && git commit && git push` 对外发布系列前，必须先向用户确认（给出明确选项：如"确认推送发布""仅本地保存不推送"），且推送前须本地 grep 隐私字段通过；获得明确同意后再继续；未经确认不得自主推送。
...
- 不要用伪精确数字：历史回报倍数必须用累计投入口径、持股比例看最新披露、概率加权期望值一律不算——给区间标"不可知"。
- 不要跨篇数字不一致：总市值、净利润、持股 % 全系列必须对齐，修订一处须联动改所有引用点。
- 不要覆盖旧版系列：若《看懂X》文件夹已存在，写入带日期后缀的新文件夹，不覆盖、不混放。
- 不要替读者做投资决策或预测股价：只给"情景+触发条件"，每篇末尾"不构成投资建议"不得删。
- 不要在用户未授权时把系列 git push 或对外发布：推送前须经 D4 检查点确认，且先本地 grep 隐私字段。

---

## 失败处理（如果 X 失败 → Y）

- 如果 Explore agent 跨篇一致性扫描超时/拒答 → 执行：降级为手动 Read 各篇比对关键数字，明确告知能力受限，不静默跳过。
- 如果 修订后联动检查遗漏引用点 → 执行：重新 grep 该数字/概念在全系列出现位置，逐处修正，不假定只改一处。
- 如果 推送前 grep 命中本机用户名/`/Users/`/真实姓名 → 执行：先脱敏再推送，不带着隐私字段提交。
- 如果 用户输入含糊（如未定篇数/主轴）→ 执行：按复杂度给 3-8 篇建议并请用户确认篇数与核心论点，不在未确认时自选。
